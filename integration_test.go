...
// TestClosureWithGlobals_FreeVariables tests free variables in ExecutionContext
func TestClosureWithGlobals_FreeVariables(t *testing.T) {
	script := tengo.NewScript([]byte(`
		make_counter := func() {
			count := 0
	...
	// Test free variable access and persistence
	...
	// Call multiple times to verify free variable persistence
	for i := 1; i <= 3; i++ {
		result, err := ctx.Call(counterFn)
		require.NoError(t, err)
		require.Equal(t, int64(i), result.(*tengo.Int).Value)
	}
}

// TestClosureWithGlobals_NestedClosures tests nested closures with mixed access patterns
func TestClosureWithGlobals_NestedClosures(t *testing.T) {
	script := tengo.NewScript([]byte(`
		global_multiplier := 10
		
		make_calculator := func(base) {
			return func(x) {
				local_const := 5
				return func() {
					// Access: global, free variable (base), local constant (x)
					return global_multiplier * base + local_const + x
				}
			}
		}
		
		calc_factory := make_calculator(3)
		calculator := calc_factory(7)
	`))
	...
	// Test nested closure with mixed access (globals, free variables, constants)
	calculatorVar := compiled.Get("calculator")
	require.NotNil(t, calculatorVar)

	calculatorFn, ok := calculatorVar.Value().(*tengo.CompiledFunction)
	require.True(t, ok)

	result, err := ctx.Call(calculatorFn)
	require.NoError(t, err)

	// Expected: 10 * 3 + 5 + 7 = 42
	require.Equal(t, int64(42), result.(*tengo.Int).Value)
}
